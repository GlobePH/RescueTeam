$(function() {
	...
	function ApprovalViewModel() {
	    ...
	    $.getJSON("http://localhost:8080/rescueph/rest/members", function(members) {
	    ...
	    self.removeUser = function(message, event) {
			...
			bootbox.confirm({
			    ...
			    callback: function(result){
			    	if(result) {
						var url = 'http://localhost:8080/rescueph/rest/members/delete/'+userId;
						$.ajax({
							url: url,
							type: 'POST',
...
//						        self.doPaging(self.pageSize(), self.currPage());
		...
		self.approveUser = function(message, event) {
			...
			bootbox.confirm({
			    ...
			    callback: function(result){
			    	if(result) {
						var url = 'http://localhost:8080/rescueph/rest/members/update/'+userId+'/true';
						$.ajax({
							url: url,
							type: 'POST',
							headers: {'Access-Control-Allow-Origin':'*'},
