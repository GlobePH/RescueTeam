$(function() {
	...
	function EmergencyViewModel() {
	    ...
	    $.getJSON("http://192.168.171.235:8080/rescueph/rest/messages", function(messages) {
	    ...
	    self.sendHelp = function(message, event) {
			...
			bootbox.confirm({
			    ...
			    callback: function(result){
			    	if(result) {
						var url = 'http://192.168.171.235:8080/rescueph/rest/messages/update/'+messageId+'/1';
						$.ajax({
							url: url,
							type: 'POST',
...
//						        self.doPaging(self.pageSize(), self.currPage());
	    ...
	    self.closeCase = function(message, event) {
			...
			bootbox.confirm({
			    ...
			    callback: function(result){
			    	if(result) {
						var url = 'http://192.168.171.235:8080/rescueph/rest/messages/update/'+messageId+'/2';
						$.ajax({
							url: url,
							type: 'POST',
							headers: {'Access-Control-Allow-Origin':'*'},
